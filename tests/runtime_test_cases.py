...
class AgentRuntimeTests(unittest.TestCase):
    ...
    def test_guide_exact_line_follow_up_returns_local_line_clues(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            workspace = Path(tmpdir)
            (workspace / "README.md").write_text(
                "Overview\nSetup\nRun command\nTroubleshooting\n",
            ...
            self.assertIn("Exact line clues:", result)
            self.assertIn("README.md:2", result)
    ...
    def test_real_traditional_chinese_trace_prompt_detection(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            workspace = Path(tmpdir)
            target = workspace / "crush_py" / "store"
            target.mkdir(parents=True)
            ...
            prompt = "追蹤 session_id 在 crush_py/store/session_store.py 的流向"

            self.assertTrue(runtime._is_direct_file_trace_prompt(prompt))
            self.assertEqual(runtime._prompt_direct_trace_variable(prompt), "session_id")
...
AgentRuntimeTests.__test__ = False
